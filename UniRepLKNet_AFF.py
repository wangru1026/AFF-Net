...
class MonaOp(nn.Module):
    def __init__(self, in_features,reduction=16):
        ...
        # 通道注意力（SE模块）
        self.fc1 = nn.Linear(in_features, in_features // 4)   # in_features = 输入特征通道数
    ...
    def forward(self, x):
        ...
        # 多尺度特征拼接
        #x = torch.cat([conv1_x, conv2_x, conv3_x], dim=1)  # [B, 3*C, H, W]
...
class Mspa(nn.Module):
    def __init__(self, in_dim, factor=4):
        super().__init__()
        hidden_dim = max(1, in_dim // factor)   # 自动算 bottleneck

        self.project1 = nn.Conv2d(in_dim, hidden_dim, kernel_size=1)
        self.nonlinear = F.gelu
        self.project2 = nn.Conv2d(hidden_dim, in_dim, kernel_size=1)

        self.adapter_conv = MonaOp(hidden_dim)
        self.norm = nn.GroupNorm(1, in_dim)
        self.gamma = nn.Parameter(torch.ones(1, in_dim, 1, 1) * 1e-6)
        self.gammax = nn.Parameter(torch.ones(1, in_dim, 1, 1))

    def forward(self, x):
        identity = x
        x = self.norm(x) * self.gamma + x * self.gammax
        h = self.project1(x)
        h = self.adapter_conv(h)
        h = self.nonlinear(h)
        h = self.project2(h)
        return h + identity
...
class UniRepLKNetBlock(nn.Module):
    ...
    def compute_residual(self, x):
        y = self.my_module_1(self.norm(self.dwconv(x)))
...
class UniRepLKNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.norm0 = BatchNorm2d(dims[0], eps=1e-6)
        self.norm1 = BatchNorm2d(dims[1], eps=1e-6)
        self.norm2 = BatchNorm2d(dims[2], eps=1e-6)
        self.norm3 = BatchNorm2d(dims[3], eps=1e-6)
    ...
    def forward_features(self, x):
        ...
        x = self.stages[0](x)
        x = self.norm0(x)  # 对应 stage 0 的通道数归一化
        outs.append(x)
    
        # ===== Stage 2 =====
        x = self.downsample_layers[1](x)
        x = self.stages[1](x)
        x = self.norm1(x)  # 对应 stage 1 的通道数归一化
        outs.append(x)
    
        # ===== Stage 3 =====
        x = self.downsample_layers[2](x)
        x = self.stages[2](x)
        x = self.norm2(x)  # 对应 stage 2 的通道数归一化
        outs.append(x)
    
        # ===== Stage 4 =====
        x = self.downsample_layers[3](x)
        x = self.stages[3](x)
        x = self.norm3(x)  # 对应 stage 3 的通道数归一化
        outs.append(x)
...
LOCAL_PRETRAINED_DIR = "/root/pretrained_ckpt/pvt"  # 替换成你本地权重所在路径

huggingface_file_names = {  "unireplknet_s_1k": "unireplknet_s.pth",}

def load_with_key(model, key):
    # 直接使用本地路径加载
    if key not in huggingface_file_names:
        raise ValueError(f"No local pretrained file for key {key}")
    
    checkpoint_path = os.path.join(LOCAL_PRETRAINED_DIR, huggingface_file_names[key])
...
def initialize_with_pretrained(model, model_name, in_1k_pretrained=False, in_22k_pretrained=False, in_22k_to_1k=False):
    if in_1k_pretrained:
        key = model_name + '_1k'
...
@register_model
def unireplknet_s(in_1k_pretrained=False, in_22k_pretrained=False, in_22k_to_1k=False, freeze_norm=False, **kwargs):
    # 1. 创建模型
    model = UniRepLKNet(depths=UniRepLKNet_N_depths, dims=(96, 192, 384, 768), **kwargs)
